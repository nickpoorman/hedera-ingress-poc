// Package bindport makes it easy to bind to a series of unused ports.
package bindport
...
// func GetListeners(n int) []net.Listener {
// 	ports, err := GetListenersWithErr(n)
// 	if err != nil {
// 		panic(err)
// 	}
// 	return ports
...
// GetS returns n ports as strings that are free to use, panicing if it can't
// succeed.
...
func GetWithErr(n int) (ports []int, err error) {
	...
	if n > blockSize-1 {
		return nil, fmt.Errorf("bindport: block size is too small for ports requested")
	}
...
func initialize() {
	if lowPort+maxBlocks*blockSize > maxPorts {
		panic("bindport: block size is too big or too many blocks requested")
	}
	...
	panic("bindport: can't allocated port block")
}
